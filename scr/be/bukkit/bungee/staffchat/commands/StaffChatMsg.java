...
public class StaffChatMsg extends Command {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (!(args.length >= 2)) {
            sender.sendMessage(ChatColor.translateAlternateColorCodes('&', Main.getLang().getString("msg-invalid-args")));
            return;
        }
        ProxiedPlayer target = Main.bsc.getProxy().getPlayer(args[0]);
        if (target == null) {
            sender.sendMessage(ChatColor.translateAlternateColorCodes('&', Main.getLang().getString("msg-given-player-offline").replaceAll("%target%", args[0])));
            return;
        }
        String msg;
