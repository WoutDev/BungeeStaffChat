...
public class PlayerJoin implements Listener {
    ...
    public void onPlayerJoin(PostLoginEvent e) {
        if (e.getPlayer().hasPermission("staffchat.update") || e.getPlayer().hasPermission("staffchat.*")) {
            if (checkForUpdate()) {
                e.getPlayer().sendMessage(ChatColor.translateAlternateColorCodes('&', "&6There is a newer version of &3BungeeStaffChat &6available! v&2" + Main.checkedVersion + " &6is downloadable now!"));
                return;
            }
        }
    }
    ...
    private boolean checkForUpdate() {
        ...
        try {
            v = new Scanner(new URL("http://www.woutdev.be/bungeestaffchat/version.html").openStream(), "UTF-8").useDelimiter("\\A").next();
        } catch (IOException e) {
            e.printStackTrace();
        }
