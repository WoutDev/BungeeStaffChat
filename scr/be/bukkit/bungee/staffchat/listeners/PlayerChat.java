...
public class PlayerChat implements Listener {
    ...
    public void onPlayerChat(ChatEvent event) {
        ...
        if (ppcon instanceof ProxiedPlayer) {
            ...
            if ((StaffChatToggle.toggledPlayers.contains(pp.getUniqueId())) && (!event.isCommand()) && ((pp.hasPermission("staffchat.use")) || (pp.hasPermission("staffchat.*")))) {
                Main.bsc.getProxy().getPluginManager().dispatchCommand(pp, Main.getConfig().getString("command-for-sc").substring(1) + " " + event.getMessage());
