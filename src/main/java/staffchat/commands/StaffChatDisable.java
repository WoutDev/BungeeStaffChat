    ...
    public void execute(CommandSender sender, String[] args)
    {
        if (sender instanceof ProxiedPlayer)
        {
            ...
            if (pp.hasPermission("staffchat.disable") || pp.hasPermission("staffchat.*"))
            {
                ...
                {
                    pp.sendMessage(
                            ChatColor.translateAlternateColorCodes('&', Main.getLang().getString("enabled-staffchat")));
                    disabled.remove(pp.getUniqueId());
                    return;
                }
                pp.sendMessage(
                        ChatColor.translateAlternateColorCodes('&', Main.getLang().getString("disabled-staffchat")));
        ...
        sender.sendMessage(ChatColor.translateAlternateColorCodes('&', Main.getLang()
                                                                           .getString(
                                                                                   "console-trys-to-disable-staffchat")));
    }
}
